...
const AddressCard: React.FC<AddressCardProps> = ({ country, phone, email, location, mapUrl }: AddressCardProps) => {
    ...
    return (
        <div className="relative w-full h-full flex flex-col">
            ...
            {/* Card Content */}
            <div className="address_details border rounded-2xl border-[#45C2CC] flex flex-col justify-between h-full p-6 bg-[#1d1d1dc6]">
                <div className="flex flex-col gap-3">
                    <Link href={`tel:${phone?.trim().split(' ').join('') || ''}`} className="flex gap-2 items-center">
                    ...
                    <Link href={mapUrl || ""} className="flex gap-2 items-start">
