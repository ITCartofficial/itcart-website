                ...
                <div className="mx-auto w-full">
                    <div className={`grid gap-6 sm:grid-cols-2 lg:grid-cols-${itemePerRow ? itemePerRow : 3}`}>
                        {(serviceCardItems ?? []).map((feature, index) => (
                            ...
                            >
                                <div className="group mb-3 inline-flex h-16 w-16 group-hover:bg-white items-center justify-center rounded-full bg-[#2BADB7] overflow-hidden transition-all duration-300 hover:bg-white">
                                    <Image
                                        src={feature?.icon || "/fallback-icon.png"}
                                        alt={feature?.title || "icon"}
                                        width={32}
                                        height={32}
                                        className="object-contain invert transition-all duration-300 group-hover:invert-1 group-hover:brightness-0 "
                                    />
                                </div>
