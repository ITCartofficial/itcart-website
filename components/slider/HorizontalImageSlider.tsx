    ...
    <div className="w-full mx-auto px-4 relative">
      ...
      <Swiper
        ...
        breakpoints={{
          ...
          1024: { slidesPerView: 2.3, spaceBetween: 60 },
          1280: { slidesPerView: 2.8, spaceBetween: 80 },
        }}
        className="!pb-12"
      >
        {slideImages.map((image, index) => (
          <SwiperSlide key={index}>
            <div className="relative w-full max-w-[700px] h-64 md:h-[350px] lg:h-[430px] rounded-lg overflow-hidden shadow-lg">
              <Image
                src={image}
                alt={`Slide ${index + 1}`}
                fill
        ...
        <div className="swiper-button-prev !text-blue-500 after:!text-2xl !left-2 !top-1/2 !-translate-y-1/2"></div>
        <div className="swiper-button-next !text-blue-500 after:!text-2xl !right-2 !top-1/2 !-translate-y-1/2"></div>
      </Swiper>

      {/* Pagination */}
      <div className="absolute left-1/2 bottom-0 transform -translate-x-1/2 flex space-x-2 z-10">
        {slideImages.map((_, index) => (
          <button
            key={index}
            onClick={() => swiperRef.current?.slideToLoop(index)}
