...
const Navigation: React.FC<NavigationProps> = ({ className, isMobile, onLinkClick }) => {
  ...
  return (
    <nav className={className || ''}>
      <ul className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-start ${
        isMobile ? '' : 'lg:items-center'
      } ${
        isMobile ? 'gap-0' : 'gap-2 lg:gap-3'
      } relative z-50 ${isMobile ? '' : 'whitespace-nowrap'}`}>
        {navItems.map((item, i) => (
          <li
            key={i}
          ...
          >
            <NavLink 
              ...
              isMobile={isMobile}
              isSubmenuOpen={isMobile ? openMobileSubmenu === item.label : false}
