        ...
        <div className={`flex flex-col gap-4 items-center ${className}`}>
            {
                descriptions && descriptions.map((text, i) => (
                    <BodyText key={i} text={text.trim()} className='lg:text-[20px] font-normal text-white w-[80%]' />
                )) || <BodyText text={description} className='lg:text-[20px] font-normal text-white  w-[80%]' />
            }
        </div>
    )
}
