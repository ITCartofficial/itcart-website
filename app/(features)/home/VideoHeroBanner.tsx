...
type VideoHeroBannerProps = {
    ...
    gradiuntClass?: string
};

const VideoHeroBanner = ({ className, heading, description, buttonText, gradiuntClass }: VideoHeroBannerProps) => {
        ...
        <section className={`relative w-full ${className}  border-[#303030]  `}>
            ...
            >
                <div className="h-full w-full flex flex-col items-center lg:items-start justify-between gap-6 pt-60 pb-16 lg:pt-4 lg:pb-24 lg:translate-y-[50%]">
                    {/* <GradientTitle text={heading} className={` leading-[1.3] text-center lg:text-left  ${gradiuntClass ? gradiuntClass : "text-[30px] lg:text-[64px] w-[70%]"}`} /> */}
                    <GradientTitle text={heading} as="h1"className={`leading-[1.3] text-center lg:text-left ${gradiuntClass ? gradiuntClass : "text-[30px] lg:text-[64px] w-[70%]"}`} 
                    />
